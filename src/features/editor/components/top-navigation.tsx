...
const Tab = ({fileId,isFirst,projectId}:{
    ...
    const {setActiveTab,openFile,closeTab,activeTabId,previewTabId} = useEditor(projectId)
    ...
    return (
        ...
        className={cn("flex items-center gap-2 h-8.75 pl-2 pr-1.5 cursor-pointer text-muted-foreground group border-y border-x border-transparent hover:bg-accent/30", isActive && "bg-background text-foreground border-x-border border-b-background -mb-px drop-shadow-2xl",isFirst && "border-l-transparent!")}
        >
            {
                file === undefined ?(
