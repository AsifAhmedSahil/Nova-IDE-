...
// this is use for breadcrumbs navigation (src > components > button.tsx)
...
export const deleteFile = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const deleteRecursive  = async(fileId: Id<"files"> ) =>{
        ...
        // if it is a folder then delete all children 1st
        if(item.type === "folder"){
          const children = await ctx.db.query("files").withIndex("by_project_parent",(q)=>q.eq("projectId",item.projectId).eq("parentId",fileId))
          .collect()
        ...
        // delete storage file if it is exists
        if(item.storageId){
